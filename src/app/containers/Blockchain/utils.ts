...
import { SendTransactionsPropsType, TransactionsDisplayInfoType } from "@multiversx/sdk-dapp/types/transactions.types"
...
export function* runQuery({ query, endpointDefinition }: { query: Query; endpointDefinition: EndpointDefinition }) {
  const queryHandler: BlockchainState["queryHandler"] = yield select(BlockchainDomains.queryHandler)
  if (queryHandler) {
