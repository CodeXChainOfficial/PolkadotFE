...
function* initializeQuerHandler(action: PayloadAction<BlockchainState["network"] | undefined>) {
  if (action.payload) {
    const {
      network: { apiAddress },
    } = action.payload
    const proxy = new ProxyNetworkProvider(apiAddress)
    const queryHandler: BlockchainState["queryHandler"] = yield call(getQueryHandler, proxy)
    if (queryHandler) {
      yield put(BlockchainActions.setQueryHandler(queryHandler))
    } else {
      console.error("Error: queryHandler is undefined")
...
function* getxNamesTransactions() {
  ...
  const account: BlockchainState["connectedAccount"] = yield select(BlockchainDomains.connectedAccount)
  const ntwrk: BlockchainState["network"] = yield select(BlockchainDomains.network)
  if (ntwrk && account) {
    const {
      network: { apiAddress },
    } = ntwrk
    ...
    try {
      ...
      yield put(BlockchainActions.setxNamestransactionsFetchingError(error.message))
    } finally {
...
function* resetsetXNamesTransactionStatus() {
  const xNamesTransactionStatus: BlockchainState["xNamesTransactionStatus"] = yield select(
...
export function* blockchainSaga() {
  ...
  yield takeLatest(BlockchainActions.getxNamesTransactions.type, getxNamesTransactions)
  yield takeLatest(BlockchainActions.setXNamesTransactionStatus.type, resetsetXNamesTransactionStatus)
}
