import { LoginInfoStateType, NetworkConfigStateType } from "@multiversx/sdk-dapp/reduxStore/slices"
...
const blockchainSlice = createSlice({
  ...
  reducers: {
    ...
    setNetwork(state, action: PayloadAction<NetworkConfigStateType | undefined>) {
      state.network = action.payload
    },
    setLoginInfo(state, action: PayloadAction<LoginInfoStateType | undefined>) {
      state.loginInfo = action.payload
    },
    setQueryHandler(state, action: PayloadAction<BlockchainState["queryHandler"]>) {
      state.queryHandler = action.payload
    },
    // xNames actions
    getxNamesTransactions(state, action: PayloadAction<void>) {},
    setXNamesTransactionStatus(state, action: PayloadAction<ContainerState["xNamesTransactionStatus"]>) {
    ...
    setxNamesTransactions(state, action: PayloadAction<ServerTransactionType[]>) {
      state.xNamesTransactions = action.payload
    },
    setxNamestransactionsFetchingError(state, action: PayloadAction<string | undefined>) {
      state.xNamestransactionsFetchingError = action.payload
    },
  },
})

export const { actions: BlockchainActions, reducer: BlockchainReducers, name: sliceKey } = blockchainSlice
