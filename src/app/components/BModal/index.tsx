...
export const BModal: FC<BModalProps> = ({ isOpen, onClose, modalsize, children, title }) => {
  return (
    <BDialog TransitionComponent={Transition} fullScreen={false} open={isOpen} onClose={() => onClose()}>
      <Wrapper>
        <Header>
          <BTitle color={CssVariables.White} btnSize="h5">
            {title}
          </BTitle>
          <CloseWrapper onClick={() => onClose()}>{/* <CloseSquareIcon /> */}</CloseWrapper>
        </Header>
