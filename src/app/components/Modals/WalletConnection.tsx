...
import { ArrowRight, CryptoxIcon, CustomLink, HighlightedBox, StyledText } from "./style"
...
const AndroidLink = "https://play.google.com/store/apps/details?id=wallet&hl=en_IE&gl=US"
...
export const ConnectWalletInfo = () => {
  ...
  const onClose = () => dispatch(globalActions.setIsWalletConnectModalOpen({ open: false }))

  useEffect(() => {
    const userAgent = navigator.userAgent.toLowerCase()

    if (userAgent.search(deviceIphone) > -1 || userAgent.search(deviceIpod) > -1) setWalletAppAddress(AppleLink)

    if (userAgent.search(deviceAndroid) > -1) setWalletAppAddress(AndroidLink)
  }, [])

  return (
    <BModal isOpen={isOpen} onClose={onClose} modalsize="medium" title="Connect Your Wallet">
      <StyledText>Hey there! In order to use this website you'll need a connected wallet</StyledText>
