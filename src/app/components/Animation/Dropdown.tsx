...
export const DropdownAnimation = ({ children, className, reveal, isEmpty, emptyContent }: Props) => {
...
export const useAnimation = (ref: RefObject<HTMLDivElement>, reveal = false) => {
  ...
  useEffect(() => {
    ...
    if (reveal) {
      ...
      const tl = gsap.timeline({
        onReverseComplete: handleCanUnmount, // Now you can unmount.
        defaults: { duration: 0.4 },
      })

      animRef.current = tl

      tl.fromTo(el, { scaleY: 0, opacity: 0 }, { scaleY: 1, transformOrigin: "top center", opacity: 1, duration: 0.3 })
    } else {
